...
// ─── ADDED PASSWORD RESET SCREEN COMPONENT ────────────────────────────────────
const PasswordResetScreen = ({ onSuccess }) => {
  ...
  const handleUpdatePassword = async () => {
    ...
    setLoading(true);
    try {
      // Extract access token from the URL hash generated by Supabase redirection
      const hashParams = new URLSearchParams(window.location.hash.substring(1));
      const accessToken = hashParams.get("access_token");

      if (!accessToken) {
        throw new Error("Invalid or expired session token. Please request another reset link.");
      }
      ...
      const d = await r.json();
      if (!r.ok) throw new Error(d.msg || d.error_description || "Could not reset password.");

      setStatusMsg("Password updated successfully! Redirecting...");
      setTimeout(() => {
        onSuccess();
      }, 2000);
    } catch (e) {
      setError(e.message || "An issue occurred while changing your password.");
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div className="glass animate-in" style={{ width: "100%", maxWidth: 400, padding: 36 }}>
        <h2 className="display" style={{ fontSize: 24, fontWeight: 700, marginBottom: 6 }}>Create New Password</h2>
        <p style={{ color: T.whiteDim, fontSize: 13, marginBottom: 24 }}>Secure your workspace account with a brand new password credentials entry</p>

        {error && <div style={{ background: "rgba(239,68,68,0.1)", border: `1px solid ${T.danger}`, color: T.danger, borderRadius: 10, padding: "10px 14px", fontSize: 13, marginBottom: 18 }}>{error}</div>}
        {statusMsg && <div style={{ background: "rgba(34,197,94,0.1)", border: `1px solid ${T.success}`, color: T.success, borderRadius: 10, padding: "10px 14px", fontSize: 13, marginBottom: 18 }}>{statusMsg}</div>}
        ...
        <div className="col" style={{ gap: 16 }}>
          ...
          <div className="input-group">
            ...
            <input type="password" placeholder="Repeat new password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && handleUpdatePassword()} disabled={loading}/>
          </div>

          <button className="btn-primary" style={{ justifyContent: "center", marginTop: 6 }} onClick={handleUpdatePassword} disabled={loading}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (hash.includes("type=recovery")) {
      ...
      setScreen("recovery"); // Immediately intercept and redirect to recovery screen
    }
  }, []);
  ...
  useEffect(() => {
    try {
      ...
      // Only auto-restore user app state if they aren't completing a password reset link
      if (saved && !window.location.hash.includes("type=recovery")) { 
  ...
  return (
    <>
      ...
      {/* Recovery Screen Handler Integration */}
      {screen === "recovery" && (
        <PasswordResetScreen 
          onSuccess={() => {
            setIsRecoveryMode(false);
            window.location.hash = ""; // Clear hash clean
            setScreen("login");
          }}
        />
      )}
